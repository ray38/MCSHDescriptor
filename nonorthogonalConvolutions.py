...
def matrix_convolve4(image,kernel):
    ...
    print("convolution done, tool: {} s".format(time.time()-start))

    return result

def matrix_convolve_carve(image,kernel):

    #print(np.isnan(np.sum(kernel)))
    old_shape_x, old_shape_y, old_shape_z = image.shape
    x_third = int(round(old_shape_x / 3. ,0))
    y_third = int(round(old_shape_y / 3. ,0))
    z_third = int(round(old_shape_z / 3. ,0))
    ...
    ry = ny//2
    rz = nz//2
    result = np.zeros((x_third, y_third, z_third))
    for i in range(nx):
        for j in range(ny):
            for k in range(nz):
                result += kernel[i,j,k] * image[x_third+i-rx:2*x_third+i-rx,y_third+j-ry:2*y_third+j-ry,z_third+k-rz:2*z_third+k-rz]
    return result
...
def matrix_convolve3(image,kernel):
    ...
    extended_image = np.tile(image,(3,3,3))
    result = matrix_convolve_carve(extended_image,kernel)
    print("convolution done, tool: {} s".format(time.time()-start))
    return result
...
def calc_MC_surface_harmonic_legendre_stencil_n(hx, hy, hz, r, l, n, legendre_order, accuracy = 5, U = None):
    ...
    eval_result = MC_surface_spherical_harmonic_n_np(x, y, z, l, n, r_array)
    eval_result = np.multiply(eval_result, legendre_array)
